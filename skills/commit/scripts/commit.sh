#!/bin/bash
# commit — Portable Draft-and-Approve commit workflow
# Usage:
#   commit.sh                       Show the ritual guide
#   commit.sh --draft "msg"         Validate a draft and write commit.draft
#   commit.sh --draft "msg" --ref "Fixes #42"   Append a reference to the draft
#   commit.sh --check                Run lint on an existing commit.draft
...
function show_guide() {
    cat <<'EOF'
commit — Draft-and-Approve commit ritual
...
Step 3: Generate a validated commit draft
    ...
    * bullet: additional distinct information"
    ...
    bash skills/commit/scripts/commit.sh --draft "..." --ref "Fixes #42"
...
if bash "$LINT_SCRIPT" "$TMP_FILE"; then
    ...
    echo ""
    echo "🛑 LINT FAILED: The commit message violates commit guidelines."
