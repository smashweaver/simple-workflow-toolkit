...
*   **Planning Loop**: Populating internal task sections (Implementation Plan, Tactical Roadmap Protocol) and identifying documentation targets.
...
The implementation cycle where surgical edits are made. It is governed by the internal Tactical Roadmap Protocol and the task checklists. Automated tests are run via `swt.sh test` to provide physical evidence of correctness.

> 📊 **Tactical Visibility**: To ensure **HITL-friendly automation**, the internal Tactical Roadmap is automatically surfaced in the `/swt:flow status` report. Agents are mandated to run a status check after every tactical chunk update to verify alignment with the user.
